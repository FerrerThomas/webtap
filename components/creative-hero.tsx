  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      constructor(x: number, y: number) {
        ...
        // Create a gradient from yellow to red (0-60 range approx)
        const hue = Math.random() * 60 // 0-60 range for red, orange, yellow
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particlesArray.length; i++) {
        ...
        for (let j = i; j < particlesArray.length; j++) {
          ...
          if (distance < 30) {
            ctx.beginPath()
            ctx.strokeStyle = `rgba(255, 120, 50, ${0.4 - distance / 150})`
            ctx.lineWidth = 0.5
